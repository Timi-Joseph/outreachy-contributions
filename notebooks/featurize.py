...
dili_smiles = [smi for smi in dili_all["Drug"].tolist() if isinstance(smi, str) and smi] # and smi removes falsey values ("" or None)
dili_labels = [lbl for smi, lbl in zip(dili_all["Drug"], dili_all["Y"]) if isinstance(smi, str) and smi]
...
# Saving dili
dili_features_df = pd.DataFrame(dili_features, columns=['PCA1', 'PCA2', 'PCA3', 'PCA4', 'UMAP1', 'UMAP2', 'tSNE1', 'tSNE2'])
dili_features_df['SMILES'] = dili_smiles
dili_features_df['Label'] = dili_labels
dili_features_df.to_csv("./data/dili/dili_ccsign_features.csv", index=False)
